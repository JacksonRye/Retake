...
export const PixelWrapper: React.FC<{
  ...
  // 1. Entrance animation spring (0s -> 0.4s)
  ...
  // 2. Exit animation spring (after durationSeconds, e.g. 3s)
  const exitFrame = fps * durationSeconds;
  ...
  // Combined scale and opacity lifecycle
  const scale = frame > exitFrame ? Math.max(0, 1 - exitSpring) : enterSpring;
  const opacity = frame > exitFrame ? Math.max(0, 1 - exitSpring) : Math.min(1, enterSpring * 1.5);
